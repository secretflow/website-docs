...
我们引入了 [DistData](spec.mdx#id3) 来表示隐私保护应用程序中的输入和输出
...
对于原子属性，首先需要在 [AttributeDef](spec.mdx#id13) 中使用”type”字段进行指定。
...
- AT\_INT
- AT\_STRING
- AT\_BOOL 和相应的标量列表类型。您也可以查看 [AttrType](spec.mdx#id18) 。
...
IoDef用于指定组件的输入或输出要求。您应该使用types来声明接受的 [DistData](spec.mdx#id3) 的类型。
...
[Attribute](spec.mdx#id12) 用于表示属性的值。请使用is\_na来明确表示值为n/a。
...
- 如果您选择名为e的子元素，则attr是一个[Attribute](spec.mdx#id12) ，其中s字段为e，即子元素选择是一个字符串。
- 如果您选择名为f的子元素，则attr是一个[Attribute](spec.mdx#id12) ，其中s字段为f。
...
您应该使用 [Attribute](spec.mdx#id12) 的ss字段列出所选列。
