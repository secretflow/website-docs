...
- The <strong>private semantic</strong>, a computing node may have private data manipulations to accelerate MPC computation, for example, in <em>HESS protocol</em>, we can do <code>HShare x Private</code> without online communication.
- The <strong>zero share data infeed</strong>, when a data provider tries to share data cross nodes, it can use <code>ZeroShare + Private</code> trick to avoid online communication.
