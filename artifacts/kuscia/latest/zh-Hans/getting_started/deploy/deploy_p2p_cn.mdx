...
启动节点，默认会在当前目录下创建 kuscia-autonomy-$\{DOMAIN\_ID}-certs 目录用来存放节点的公私钥和证书：
...
将 alice 的公钥拷贝到 bob 的机器上的 $\{DOMAIN\_CERTS\_DIR} 目录中并重命名为 alice.domain.csr：
...
将 alice.domain.crt 和 ca.crt 拷贝至 alice 机器上的 $\{DOMAIN\_CERTS\_DIR} 目录并重命名为 domain-2-bob.crt 和 bob.host.ca.crt：
...
将 bob 的公钥拷贝到 alice 的机器上的 $\{DOMAIN\_CERTS\_DIR} 目录中并重命名为 bob.domain.csr：
...
将 bob.domain.crt 和 ca.crt 拷贝至 bob 机器上的 $\{DOMAIN\_CERTS\_DIR} 目录并重命名为 domain-2-alice.crt 和 alice.host.ca.crt：
