---
git_download_url: https://github.com/secretflow/kuscia/raw/afbd52621c2c14c0a838a0ebc4954ee1689be56a/docs/getting_started/run_secretflow_cn.md
git_last_modified_commit: 312299cba3241f8b44a899e1d8bf40a1d2a9bbcc
git_last_modified_time: '2023-11-08T21:07:54+08:00'
git_origin_url: https://github.com/secretflow/kuscia/blob/afbd52621c2c14c0a838a0ebc4954ee1689be56a/docs/getting_started/run_secretflow_cn.md
...
:target{#kuscia-kusciajob}

# Kuscia 入门教程 —— 了解 KusciaJob

你将会尝试自己配置并且提交一个作业，在这个过程中，你将会认识 Kuscia 中的重要概念 —— <strong>KusciaJob</strong>。

:target{#id1}

## 准备集群

这里假设你已经在机器上部署并且启动了一个示例集群。如果你还没有这样做，请先按照 [快速开始](quickstart_cn.mdx) 的指引启动一个集群。
...
2. 使用 [KusciaAPI](../reference/apis/summary_cn.mdx) 创建 [DomainData](../reference/apis/domaindata_cn.mdx)，得到 `domaindata_id` 。
3. SecretFlow 引擎任务需要获得合作方数据的 schema 信息，使用 [KusciaAPI](../reference/apis/summary_cn.mdx) 创建 [DomainDataGrant](../reference/apis/domaindatagrant_cn.mdx) 进行数据授权，将 alice 数据 schema 信息授权给 bob 。

bob 节点准备数据重复上述 1、2、3 操作。

:target{#configure-kuscia-job}
...
<Container type="tip">
  更多有关 KusciaJob 配置的信息，请查看 [<span>KusciaJob</span>](../reference/concepts/kusciajob_cn.mdx)。
  ...
  - `sf_input_ids`：表示 SecretFlow 输入数据 `id` ，SecretFlow 引擎会将 Kuscia 定义的输入数据 [<span>DomainData</span>](../reference/concepts/domaindata_cn.mdx) 转换成引擎所需要的 [DistData](https://www.secretflow.org.cn/docs/spec/latest/zh-Hans/spec#distdata)。
  - `sf_output_ids`：表示 SecretFlow 输出数据  `id` ，SecretFlow 引擎会将输出的 [DistData](https://www.secretflow.org.cn/docs/spec/latest/zh-Hans/spec#distdata) 转换成 Kuscia 的 [<span>DomainData</span>](../reference/concepts/domaindata_cn.mdx)。
...
详细信息请参考 [KusciaJob](../reference/concepts/kusciajob_cn.mdx) 。

:target{#kusciajob-kusciatask}
...
KusciaTask 的信息这里不再赘述，请查看 [KusciaTask](../reference/concepts/kusciatask_cn.mdx) 。
...
- 进一步阅读 [Kuscia 架构细节](../reference/architecture_cn.mdx)，了解 Kuscia 的设计思路和概念。
- 了解 [Kuscia API](../reference/apis/summary_cn.mdx)。Kuscia API 是 Kuscia 的一个更上层封装，支持更方便地将 Kuscia 集成到其他系统中。
- 了解 [多机器部署](../deployment/deploy_p2p_cn.mdx) 的更多信息。
- 尝试运行其它算法或是引擎的作业，比如 [互联互通银联 BFIA 协议作业](../tutorial/run_bfia_job_cn.mdx) 或是 [FATE 作业](../tutorial/run_fate_cn.mdx)。

<Container type="tip">
  如果你想要停止并清理入门教程使用的示例集群，可以查阅 [<span>相关指引</span>](quickstart_cn.mdx#stop)。
</Container>
