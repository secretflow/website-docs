---
git_download_url: https://github.com/secretflow/kuscia/raw/afbd52621c2c14c0a838a0ebc4954ee1689be56a/docs/reference/concepts/domaindata_cn.md
git_last_modified_commit: afbd52621c2c14c0a838a0ebc4954ee1689be56a
git_last_modified_time: '2023-12-12T11:52:31+08:00'
git_origin_url: https://github.com/secretflow/kuscia/blob/afbd52621c2c14c0a838a0ebc4954ee1689be56a/docs/reference/concepts/domaindata_cn.md
...
一个常见的用例是：隐私求交参与方分别在自己的 Domain 下创建出自己要参与计算的表，即`type`为`table`的 DomainData，然后创建一个 [KusciaJob](kusciajob_cn.mdx) 进行隐私求交任务，
...
端口，详情请参考 [Data Mesh API](../apis/datamesh/summary_cn.mdx#data-mesh-api)。
...
- `columns`：表示对于表类型的 DomainData 的列信息。仅当`type`为`table`时，该字段才存在。
  - `name`：列字段名称。
  - `type`：列字段的数据类型。该字段当前版本并非可枚举的，列字段的类型由应用算法组件进行定义和消费，Kuscia 仅作存储。
