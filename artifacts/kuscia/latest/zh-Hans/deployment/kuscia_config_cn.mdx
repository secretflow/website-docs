---
git_download_url: https://github.com/secretflow/kuscia/raw/4ded8d874369b27490181937494abb301d249f7c/docs/deployment/kuscia_config_cn.md
git_last_modified_commit: 96591ccd92e300dcd4d5f1302f149ebd4b41277b
git_last_modified_time: '2024-01-08T15:19:31+08:00'
git_origin_url: https://github.com/secretflow/kuscia/blob/4ded8d874369b27490181937494abb301d249f7c/docs/deployment/kuscia_config_cn.md
...
- `mode`: 当前 Kuscia 节点部署模式 支持 Lite、Master、Autonomy（不区分大小写）, 不同部署模式详情请参考[这里](../reference/architecture_cn.mdx)
- `domainID`: 当前 Kuscia 实例的 [节点 ID](../reference/concepts/domain_cn.mdx)， 需要符合 DNS 子域名规则要求，详情请参考[这里](https://kubernetes.io/zh-cn/docs/concepts/overview/working-with-objects/names/#dns-subdomain-names)
...
- `liteDeployToken`: 节点连接 master 的部署 token，用于节点向 master 注册证书， 只在节点第一次向 master 注册证书时有效，详情请参考[节点中心化部署](deploy_master_lite_cn.mdx)
- `masterEndpoint`: 节点连接 master 的地址，比如 [https://172.18.0.2:1080](https://172.18.0.2:1080)
- `runtime`: 节点运行时 runc、runk、runp，运行时详解请参考[这里](../reference/architecture_cn.mdx#agent)
...
如果使用 [start\_standalone.sh](https://github.com/secretflow/kuscia/blob/main/scripts/deploy/start_standalone.sh) 或者 [deploy.sh](https://github.com/secretflow/kuscia/blob/main/scripts/deploy/deploy.sh) 脚本部署的 kuscia，kuscia.yaml 文件路径默认是在以下位置（其他部署模式可以借鉴）。
