...
若你使用第三方算法镜像提交互联互通作业，强烈建议你检查镜像安全性，同时参考[安全加固方案](security_plan_cn.mdx)。
...
部署运行银联 BFIA 协议的节点，请参考 [快速入门点对点组网模式](../getting_started/quickstart_cn.mdx#p2p-network-mode)。

在执行启动集群命令时，需要新增一个命令行参数<code>-p bfia</code>，详细命令如下：
...
:target{#configure-bfia-kuscia-job}
...
- KusciaJob 的名称为 job-ss-lr，在一个 Kuscia 集群中，这个名称必须是唯一的，由<code>.metadata.name</code>指定。
...
KusciaJob 中算子参数由<code>taskInputConfig</code>字段定义，对于不同的算子，算子的参数不同
...
通过指定<code>-o yaml</code>参数，我们能够以 Yaml 的形式看到 KusciaJob 的详细状态。job-ss-lr 是提交的作业名称。
...
- <code>status</code>字段记录了 KusciaJob 的运行状态，<code>.status.phase</code>字段描述了 KusciaJob 的整体状态，而 <code>.status.taskStatus</code>则描述了包含的 KusciaTask 的状态。
...
KusciaJob 中的每一个 KusciaTask 都有一个<code>taskID</code>，通过<code>taskID</code>我们可以查看 KusciaTask 的详细状态。

```shell
kubectl get kt job-ss-lr-26e3489ac66e -o yaml
```

KusciaTask 的介绍，请参考 [KusciaTask](../reference/concepts/kusciatask_cn.mdx)。

:target{#id10}

## 查看 SS-LR 算子运行结果

可以通过 [查看 KusciaJob 运行状态](#get-kuscia-job-phase) 查询作业的运行状态。 当作业状态 PHASE 变成 <code>Succeeded</code> 时，可以查看算子输出结果。
...
:target{#ss-lr-task-input-config}
...
- <code>name</code>描述了任务算子的名称。
- <code>module\_name</code>描述了任务算子所属模块名称。
- <code>input</code>描述了任务算子的输入，若任务不依赖其他任务的输出，则可以将该项置为空。
- <code>output</code>描述了任务算子的输出。
- <code>role</code>描述了任务的角色。
- <code>initiator</code>描述了任务发起方的信息。
- <code>task\_params</code>描述了任务算子依赖的参数。

:target{#bfia-create-job-req-body}
...
- <code>job\_id</code>描述了作业的标识。
- <code>dag</code>描述了作业的组件之间组合的配置。
- <code>config</code>描述了作业运行时的参数配置。
