...
我们请求[创建 Job](../reference/apis/kusciaapis/kusciajob_cn.mdx#create-job) 接口来创建并运行这个 KusciaJob。
...
具体字段数据格式和含义请参考[创建 Job](../reference/apis/kusciaapis/kusciajob_cn.mdx#create-job) ，本文不再赘述。
...
我们请求[批量查询 Job 状态](../reference/apis/kusciaapis/kusciajob_cn.mdx#batch-query-job-status)接口来批量查询 KusciaJob
的状态。
...
和[批量查询 Job 状态](../reference/apis/kusciaapis/kusciajob_cn.mdx#batch-query-job-status)

:target{#id7}

## 删除 KusciaJob

当你想清理这个 KusciaJob 时，我们请求[删除 Job](../reference/apis/kusciaapis/kusciajob_cn.mdx#delete-job) 接口来删除这个
KusciaJob.
