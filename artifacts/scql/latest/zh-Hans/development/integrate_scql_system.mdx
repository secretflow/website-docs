...
- `/GetTableMeta` ：获取表的元数据，全局资源管理器服务器（GRM server）应监听 `${GRMServerHost}/GetTableMeta`
- `/GetEngines` ：获取特定 SCQLEngines（参与者）的端点，全局资源管理器服务器（GRM server）应监听 `${GRMServerHost}/GetEngines`
- `/VerifyTableOwnership` ：检查用户是否拥有表的所有权，全局资源管理器服务器（GRM server）应监听 `${GRMServerHost}/VerifyTableOwnership`
