...
- 支持多方（N>=2）。
- 支持常见的 SQL select 语法和函数，满足大多数场景的需求。详情请查看 [SCQL 实现进度](../reference/scql_status.mdx)。
...
一个 SCQL 系统由一个 SCDB 服务器和多个 SCQLEngines 组成。
...
- GRM 是 Global Resource Manager 的缩写，它管理着参与方和共享的表结构信息。
...
- <DefinitionList>
    <dl>
      ...
      <dd>
        参与方 Bank1 在其本地数据库 <code>DB1</code> 中有张表 <code>bank\_1</code>，该数据表有两列分别是 <code>customer\_id</code> 和 <code>deposit</code>。
      </dd>
    </dl>
  </DefinitionList>
- <DefinitionList>
    <dl>
      ...
      <dd>
        参与方 Bank2 在其本地数据库 <code>DB2</code> 中有张表 <code>bank\_2</code>，该数据表有两列分别是 <code>customer\_id</code> 和 <code>loan</code>。
      </dd>
    </dl>
  </DefinitionList>

<Container type="note">
  表的元数据，例如表的所有者和表的结构，需要在 GRM 中注册，以便 SCDB 可以获取表结构并解析表引用。例如，SCDB 会将 <code>table\_1</code> 解析为参与方 Bank1 中的 <code>DB1.bank\_1</code>。
</Container>
...
CCL 格式: <code>\<src\_column, dest\_party, constraint></code>

它表示 <code>src\_column</code> 可以被 <code>dest\_party</code> 在满足 <code>constraint</code> 的条件下访问。
...
<Container type="note">
  要了解更多关于 CCL 的信息，请阅读文档 [<span>SCQL CCL</span>](../reference/scql_ccl_intro.mdx)。
</Container>
