...
<Container type="note">
  ...
  `__ALICE_CREDENTIAL__` 用于验证 SCDB，您需要用自己的凭据替换它，对于 Bob 同样如此。在我们的示例中，我们简单的将 Alice 的凭据设置为 `credential_alice`，将 Bob 的凭据设置为 `credential_bob`。
</Container>
...
在您的工作区创建一个名为 `toy_grm.json` 的 JSON 文件，其内容应如下所示：
...
<Container type="note">
  `__ALICE_TOKEN__` 和 `__BOB_TOKEN__` 是用于验证用户身份的字符串令牌，您应该用自己的令牌信息替换他们。在这里，它们被设置为 `token_alice` 和 `token_bob`。

  `__ENGINE_ALICE_HOST__` 和 `__ENGINE_BOB_HOST__` 代表了 Alice 和 Bob 的 IP 地址，您应该使用自己的 IP 地址信息替换这些占位符。

  `__ALICE_PORT__` 和 `__BOB_PORT__` 代表引擎服务的监听端口，并且应该于 [1.4 创建 docker-compose 文件](#create-docker-compose-file) 中指定的发布端口相匹配。这里应该是 8080。

  `__ALICE_CREDENTIAL__` 和 `__BOB_CREDENTIAL__` 用于在向引擎发送请求时识别 SCDB，并且应该与 [1.3 配置引擎](#set-engine-config) 中指定的 `engine_credential` 相匹配。这里应该是 `credential_alice` and `credential_bob`。
</Container>
...
<Container type="note">
  ...
  `__ALICE_TOKEN__` 和 `__BOB_TOKEN__` 应该与 [2.2 配置 ToyGRM](#set-toygrm) 中设置的令牌信息保持一致。这里应该设置为 `token_alice` 和 `token_bob`。
</Container>
