...
什么是 SCQL CCL？请参阅 [SCQL CCL](../reference/scql_ccl_intro.mdx)。

Grant CCL 语法，请参阅 [Grant/Revoke CCL](../reference/scql_manual.mdx#scql-grant-revoke)。
...
下面以 Alice 和 Bob 的联合分析为例进行说明，参与方 Alice 拥有表 <code>t1</code>，该表包含字段 <code>id</code>，<code>credit\_rank</code>，<code>income</code>，<code>join\_date</code> 和 <code>age</code>。参与方 Bob 拥有表 <code>t2</code>，该表包含字段 <code>id</code>，<code>age</code>，<code>order\_amount</code>，<code>is\_active</code>，<code>admin\_date</code>，<code>out\_date</code>。

如下所示，我们以三元组的形式展示 CCL 配置。

<code>\<src\_column, dest\_party, constraint></code>

这意味着 <code>dest\_party</code> 对 <code>src\_column</code> 的访问约束为 <code>constraint</code>。若参与方存在多列相同的 CCL，它将缩写为如下形式。
...
如果 Alice 不希望将 age/income 信息泄露给 Bob，那么 Alice 仅需将 group by key 的 CCL 约束设置为 <code>ENCRYPTED\_ONLY</code>。Bob 的 CCL 配置与先前配置保持一致，并按照如下设定修改 Alice 的 CCL 配置即可。
...
如果您认为数据极度敏感，那么您应当将这些数据设置为 <code>ENCRYPTED\_ONLY</code>， 这样的设定下仅能执行聚合函数的 count 操作。
