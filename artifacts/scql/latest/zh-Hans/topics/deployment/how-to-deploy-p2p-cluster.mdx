...
本文档介绍了如何使用 docker 以 P2P 模式部署 SCQL 系统，并使用 brokerctl 进行查询。它与 [P2P 快速开始](../../intro/p2p-tutorial.mdx) 基本相同，但部署在多机上。
...
请参阅：[SCQLBroker 配置选项](../../reference/engine-config.mdx#config-broker-server-options) 了解更多信息。
...
请参阅 [引擎配置选项](../../reference/engine-config.mdx#engine-config-options) 了解更多配置信息
...
<Container type="note">
  `__INTRA_PORT__`, `__INTER_PORT__` 和 `__ENGINE_PORT__` 作为监听端口，您需要替换为可访问的端口，这里我们指定为 `8080`, `8081` 和 `8003`
...
然后您需要将 party\_info.json 和 authorized\_profile.json 中的 `__XXX_PUBLIC_KEY__` 占位符替换为相应的真实公钥。
...
您可以开始使用 brokerctl 向 SCQLBroker 提交请求并获取结果。这与您在 [P2P 快速开始](../../intro/p2p-tutorial.mdx) 中可以执行的操作类似。
