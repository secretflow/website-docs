---
git_download_url: https://github.com/secretflow/scql/raw/f8a1a30688868496fb9aa22bbd75cdcfe5793e99/docs/topics/ccl/intro.rst
git_last_modified_commit: bf5789bb37114a0f45d04d938337ec1af474a847
git_last_modified_time: '2023-12-04T11:39:34+08:00'
git_origin_url: https://github.com/secretflow/scql/blob/f8a1a30688868496fb9aa22bbd75cdcfe5793e99/docs/topics/ccl/intro.rst
...
<Container type="note">
  请用户根据自己的场景需求和隐私保护需要设置 CCL。攻击者可能会构造满足 CCL 约束的 Query，通过复杂查询或者多次查询方式，从结果推测原始数据敏感信息，相关风险描述和建议见 [<span>SCQL 安全性说明</span>](../security/overview.mdx).
</Container>
...
当 JOIN keys 的 CCL constraint 均为 `PLAINTEXT_AFTER_JOIN` 或 `PLAINTEXT` 时，则对于 <strong>InnerJoin</strong> ， JOIN keys 交集结果的 CCL 变为 `PLAINTEXT` ，对于 <strong>LeftOuterJoin</strong> ，只有右 JOIN key 的交集结果的 CCL 变为 `PLAINTEXT` ， 对于 <strong>RightOuterJoin</strong> ，则是左 JOIN key 的交集结果变为 `PLAINTEXT` 。
...
- 当其中一个入参的 CCL constraint 为 `PLAINTEXT` 时，在 CCL constraint 推导时可以忽略该参数不影响结果的 CCL constraint。
