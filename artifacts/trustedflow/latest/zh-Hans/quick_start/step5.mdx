...
每一份文件都有一个ID与之对应，您可以按照步骤三的说明，找到可信APP——XGBoost训练对应的任务配置文件`xgb.json`，我们会找到输出文件的说明。 如下所示，`file://output/?id=model_uuid&&uri=/host/testdata/breast_cancer/xgb_model`说明XGBoost训练APP得到的模型ID叫做`model_uuid`，文件位置为/host/testdata/breast\_cancer/xgb\_model，该文件是加密的。我们假设alice从carol中得到了这个文件。
...
```none
cms_util generate-data-export-cert --config-file=expor.yaml --dest-file=data-export.json
```
...
- \`vote\_request\_signature\`\`: 上一步alice得到的数据导出请求的内容，拷贝到这里。记住，请替换为您实际得到的内容。
- `action`: `APPROVED`表示同意alice的请求，无需修改。
- \`private\_key\_file\`\`: bob自己的私钥，用于签名。
