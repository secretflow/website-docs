...
git_download_url: https://github.com/secretflow/trustedflow/raw/e529736dda5000fa638150b0c1992a603de524ba/docs/architecture/apps/index.rst
git_origin_url: https://github.com/secretflow/trustedflow/blob/e529736dda5000fa638150b0c1992a603de524ba/docs/architecture/apps/index.rst
...
4. 输入（inputs）和输出（outputs）定义了输入输出的类型，用来标识它是一张数据表（table）、一个机器学习的模型（model）还是一个数据分析的报告（report）等。输入输出中的属性（attrs）用来表示该输入输出的额外属性，例如求交算子中用”key”来表示输入表中的求交列，woe分箱算子中用”feature\_selects”来表示输入表选取列。
