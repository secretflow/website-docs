---
git_download_url: https://github.com/secretflow/secretflow/raw/8cf652dadee6c02cb09a0225efa6d7c6bae31246/docs/getting_started/debug_mode.md
git_last_modified_commit: b06e3ccb5bfcf23b0e982da3023a85436f82e00b
git_last_modified_time: '2023-12-25T14:23:18+08:00'
git_origin_url: https://github.com/secretflow/secretflow/blob/8cf652dadee6c02cb09a0225efa6d7c6bae31246/docs/getting_started/debug_mode.md
...
隐语框架有一个很重要的特点就是:<strong>用户只需要写一套secretflow的代码，可以在多种部署条件下直接执行！</strong>
这意味着，用户可以使用简单易部署的方式进行开发，随后<strong>零成本</strong>的迁移到生产环境产生价值。
我们的隐语目前已经提供了两种模式分别是：
...
所以，从设计角度考虑，我们完善的部署模式中还缺少一种能在本地按顺序执行、能即时反馈错误、以及支持断点调试的模式。若能补充这样一种开发模式，我们的<strong>开发</strong>、<strong>模拟</strong>和<strong>生产</strong>环境将更为完善。因此，我们计划开发一种新的模式，称为调试模式（Debug Mode）。
...
上述这些是本次我们需要重写的接口，其中最重点的是<strong>remote</strong>接口。
...
生产模式的通信网络
![Alt text](../_assets/debug_01.png)

simulation模式的通信网络
![](../_assets/debug_02.png)
debug模式结构
![Alt text](../_assets/debug_03.png)
debug模式设计
...
<strong>这些细节逻辑框架都已经通过proxy做好了，用户无需关注细节</strong>
...
<strong>接下来secretflow程序将会以debug mode来进行调度和运行。</strong>
...
3. 如果代码中有spu相关内容，由于spu的运行模式和同步模式不兼容，debug模式使用pyu进行了功能模拟，<strong>请务必使用simulation模式进行验证</strong>
