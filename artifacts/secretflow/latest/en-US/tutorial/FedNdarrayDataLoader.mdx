...
<LineBlock>
  ...
  SecretFlow supports multiple formats, including <code>.npy</code> and <code>.npz</code>, and its interface is designed to be compatible with <code>numpy</code>
</LineBlock>
...
In SecretFlow, we provide an interface similar to <code>numpy.load</code> called <code>secretflow\.load.ndarray.load</code> to load <code>ndarray</code> data from multiple parties and convert it into a federated representation.

Using secretflow\.data.load, you can read numpy files from multiple parties and create a <code>FedNdarray</code> object.
