...
<LineBlock>
  ...
  经过推导之后实际传输的数据： - (1) 的维度是`batchsize*1`,不会泄露v矩阵的信息 - (2) 的维度是batchsize\*K，对于特征维度的信息已经消去，只保留了FM vector dimension上的和信息（通常dim=4）
</LineBlock>
...
<LineBlock>
  ...
  关于DeepFM的封装在secretflow/ml/nn/applications/sl\_deep\_fm.py，提供了`DeepFMBase`和`DeepFMFuse`两个模块。
...
产出了 `alice: alice_ml1m.c™sv bob: bob_ml1m.csv`™
