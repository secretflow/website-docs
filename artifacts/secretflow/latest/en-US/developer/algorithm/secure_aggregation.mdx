...
SecretFlow provides a variety of <code>Aggregator</code> for users to choose from, each <code>Aggregator</code> provides the function of sum/average.
...
<code>Masking with One-Time Pads</code> negotiates a secret for every two participants, then uses the secret to hide its input <InlineMath>$x$</InlineMath>, and each participant outputs.
...
Note that <code>Masking with One-Time Pads</code> is based on semi-honest assumptions and does not support client dropping. For more information, please refer to [Practical Secure Aggregation for Privacy-Preserving Machine Learning](https://eprint.iacr.org/2017/281.pdf)
