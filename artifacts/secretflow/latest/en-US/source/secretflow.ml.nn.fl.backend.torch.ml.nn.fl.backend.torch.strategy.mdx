...
<Outline domain="py" objectType="class" target={"secretflow.ml.nn.fl.backend.torch.strategy.fed_avg_g.FedAvgG"} module={"secretflow.ml.nn.fl.backend.torch.strategy.fed_avg_g"} fullname={"FedAvgG"} parameters={["builder_base", "random_seed"]} description={"Bases: BaseTorchModel"}>
  ...
  <Outline.Content>
    Bases: [<code>BaseTorchModel</code>](secretflow.ml.nn.fl.backend.ml.nn.fl.backend.torch.mdx#secretflow.ml.nn.fl.backend.torch.fl_base.BaseTorchModel "secretflow.ml.nn.fl.backend.torch.fl_base.BaseTorchModel")
