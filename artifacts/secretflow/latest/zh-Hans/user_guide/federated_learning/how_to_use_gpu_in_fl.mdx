---
git_download_url: https://github.com/secretflow/secretflow/raw/f2c46f98224a42f29bca20751a569bbfac39f750/docs/user_guide/federated_learning/how_to_use_gpu_in_fl.md
git_last_modified_commit: b06e3ccb5bfcf23b0e982da3023a85436f82e00b
git_last_modified_time: '2023-12-25T14:23:18+08:00'
git_origin_url: https://github.com/secretflow/secretflow/blob/f2c46f98224a42f29bca20751a569bbfac39f750/docs/user_guide/federated_learning/how_to_use_gpu_in_fl.md
...
<em>Ray 参考文档</em>：[Ray support GPU document](https://docs.ray.io/en/latest/ray-core/tasks/using-ray-with-gpus.html)
...
从ray的例子中可以看到我们需要在两个地方设置num\_gpus：
...
1. 我们在sf.init中增加了num\_gpus的参数
   ![Alt text](../../_assets/fl_gpu_0.png)
2. 在proxy中封装num\_gpus，这样在FLModel和SLModel的dispatcher就可以把num\_gpus带到worker端，完成配置。我们在worker端只需要感知是不是要使用GPU，引擎可以获取到所有分配的gpu资源进行调度。
   ![Alt text](../../_assets/fl_gpu_1.png)

:target{#id6}

#### 那怎么使用呢

在`sf.init`中配置num\_gpus
...
<strong>Attention</strong>：在使用模拟模式（Simulation Mode）时，需要特别注意的是，如果系统中只装有一块显卡，在分配给多个参与方时，每个参与方能够获得的资源将限制为显卡总资源的 1/n。如果不遵循这一限制，可能会导致由于资源超额分配而无法成功调度。
