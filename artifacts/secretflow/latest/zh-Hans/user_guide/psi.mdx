...
:target{#id27}[\[RA18\]](#ra18) 第3章介绍了一种非平衡的PSI协议(基于ecdh-OPRF)，该协议是 :target{#id28}[\[BBCD+11\]](#bbcd-11) 对 :target{#id29}[\[JL10\]](#jl10) 恶意PSI协议进行简化得到semi-honest协议。协议分成两个阶段：离线预处理阶段和在线阶段。[\[RA18\]](#ra18) 中引入了一些优化方法减少离线预处理阶段的计算和通信量。secretflow中实现了\[RA18]\_ 第3章中的基本协议。
