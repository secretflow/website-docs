...
您应该使用 secretflow\.component.entry.comp\_eval 来执行节点。

以下代码演示了如何使用此API，但不能直接运行。
...
- log\_file: 日志文件路径
- result\_file: 结果文件路径。
- eval\_param: base64编码的NodeEvalParam prototext。
- cluster: Base64编码的SFClusterConfig prototext。
...
如果你想要在 SecretFlow 中创建一个新的组件，你可以参考 \<[https://github.com/secretflow/secretflow/blob/main/secretflow/component/preprocessing/train\_test\_split.py](https://github.com/secretflow/secretflow/blob/main/secretflow/component/preprocessing/train_test_split.py)>\`\_ 这个是最简单的组件。
