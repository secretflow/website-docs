...
TEEU(<code>TEE</code> processing <code>U</code>nit)是 SecretFlow 中的 TEE 设备，通过 TEEU，用户可以方便的把数据放在TEE内进行计算，并且达到保护数据完整和安全的目的
...
当运行结束后，您可以在 <code>/root/occlum\_instance/model.json</code> 查阅模型文件。
