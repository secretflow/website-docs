...
<LineBlock>
  BaseModule: 类似于 <code>torch.nn.module</code>

  TorchModel: 一个包装类包括 <code>loss\_fn</code>,<code>optim\_fn</code>,

  metric\_wrapper: metrics包装器

  optim\_wrapper： optim\_fn包装器

  FLModel：联合模型，使用 <code>backend</code> 指定哪个bachend将被使用，使用 <code>strategy</code> 来指定将使用哪个联合策略
</LineBlock>
...
当我们定义模型时，我们只需要继承 <code>BaseModule</code> 来代替 <code>nn.Module</code> 的，其他与pytorch一致。
