...
<Notebook.Cell>
  ...
  <Notebook.CodeArea prompt="" stderr={false} type="output">
    <pre>
      <span className="ansi-cyan-fg">{"(_run pid=3833768)"}</span>{" [2023-06-26 13:58:34.369] [info] [thread_pool.cc:30] Create a fixed thread pool with size 63\n"}<span className="ansi-cyan-fg">{"(SPURuntime(device_id=None, party=bob) pid=3839631)"}</span>{" 2023-06-26 13:58:34.779 [info] [thread_pool.cc:ThreadPool:30] Create a fixed thread pool with size 63\n"}<span className="ansi-cyan-fg">{"(SPURuntime(device_id=None, party=alice) pid=3839630)"}</span>{" 2023-06-26 13:58:34.869 [info] [thread_pool.cc:ThreadPool:30] Create a fixed thread pool with size 63\n"}
    </pre>
  </Notebook.CodeArea>
...
有时我们可能需要IV值。根据GitHub issue, [https://github.com/secretflow/secretflow/issues/565](https://github.com/secretflow/secretflow/issues/565)，发布分桶的ivs可能会泄漏标签信息。目前，我们选择将桶的iv值保存在标签持有者设备中。标签持有者可以选择 1. 不共享iv信息 2. 共享一些已选择的iv信息
...
回想一下，woe\_rules是一个字典 \{PYU: PYUObject}，其中 “ “每个 PYUObject 本身是以下类型的字典
